...
# Introduction <a href="#an_pm_intro" id="an_pm_intro"></a>
...
Please refer to the <a href="pg_syspm_users_guide.md">ADK-PM Programmers Guide</a> for ADK Power Management features, behavior and configuration.

# Operating Modes <a href="#an_pm_operating_modes" id="an_pm_operating_modes"></a>
...
|----|----|----|
| <p>Active</p> | <p>On</p> | <p>On</p> |
| <p>Standby</p> | <p>On</p> | <p>Dimmed</p> |
| <p>Sleep</p> | <p>On</p> | <p>Dimmed</p> |
| <p>Deep sleep</p> | <p>On</p> | <p>Off</p> |
| <p>Hibernate</p> | <p>Off</p> | <p>Off</p> |

## Modes Definition <a href="#an_pm_modes_definition" id="an_pm_modes_definition"></a>

### Active <a href="#an_pm_active_mode" id="an_pm_active_mode"></a>
...
### Standby <a href="#an_pm_standby_mode" id="an_pm_standby_mode"></a>
...
### Sleep <a href="#an_pm_sleep_mode" id="an_pm_sleep_mode"></a>
...
### Deep Sleep <a href="#an_pm_deepsleep_mode" id="an_pm_deepsleep_mode"></a>
...
- Any enabled wake-up source can be used in order to resume the system and transit back to the Active Mode, restoring LCD back-light to the active value.
- Please see <a href="pg_syspm_users_guide.md">ADK-PM Programmers Guide</a> to see what wake-up sources are available and how they can be configured.
- Power consumption is highly dependent on the individual Use Case. System configuration also impacts the consumption, like the enabled communication interfaces.
- The time required by the system to resume properly should always be less than 1-2 sec.

### Hibernate <a href="#an_pm_hibernate_mode" id="an_pm_hibernate_mode"></a>
...
# Recommended Operating Modes settings <a href="#an_pm_recommended_modes_settings" id="an_pm_recommended_modes_settings"></a>
...
Please refer to the <a href="pg_syspm_users_guide.md">ADK-PM Programmers Guide</a> to known how to change PM configuration parameters, by means of the ADK configuration file.

## Update to Latest software <a href="#an_pm_update_software" id="an_pm_update_software"></a>
...
## Avoid high CPU usage <a href="#an_pm_avoid_high_cpu" id="an_pm_avoid_high_cpu"></a>
...
## Adjust display settings <a href="#an_pm_adjust_display_settings" id="an_pm_adjust_display_settings"></a>
...
## Turn-off the communication interfaces <a href="#an_pm_turnoff_comint" id="an_pm_turnoff_comint"></a>
...
## Enable low power modes <a href="#an_pm_low_power_modes" id="an_pm_low_power_modes"></a>
...
### Standby Mode <a href="#an_pm_lp_standby" id="an_pm_lp_standby"></a>
...
### Sleep Mode <a href="#an_pm_lp_sleepmode" id="an_pm_lp_sleepmode"></a>
...
You can find useful information here <a href="https://www.verifone.com/en/us/insight/preventing-image-persistence">https://www.verifone.com/en/us/insight/preventing-image-persistence</a>
...
### Deep Sleep Mode <a href="#an_pm_lp_deepsleep" id="an_pm_lp_deepsleep"></a>
...
### Hibernate Mode <a href="#an_pm_lp_hibernate" id="an_pm_lp_hibernate"></a>
...
# Known issues and Limitations <a href="#an_pm_known_issues" id="an_pm_known_issues"></a>
...
# Common Use Cases <a href="#an_pm_common_use_cases" id="an_pm_common_use_cases"></a>

## Use Case I: Terminal always active and ready to transact immediately <a href="#an_pm_usecase_i" id="an_pm_usecase_i"></a>
...
## Use Case II: Light usage of the Terminal with GSM connected only when needed <a href="#an_pm_usecase_ii" id="an_pm_usecase_ii"></a>
...
## Use Case III: Medium usage of the Terminal with Display always ON <a href="#an_pm_usecase_iii" id="an_pm_usecase_iii"></a>
...
- Please note that Verifone recommends to follow some tips in order to prevent image persistence (or image retention) on a liquid crystal (LC) display. It must be a design consideration for all developers of applications that control display content. You can find useful information here <a href="https://www.verifone.com/en/us/insight/preventing-image-persistence">https://www.verifone.com/en/us/insight/preventing-image-persistence</a>. If Sleep mode is configured and expected to maintain residency in that mode for longer than what is currently recommended as per above advisory, then you need to either wake-up and go back to the same mode or transition to next low power state - Deep Sleep or Hibernate.

## Use Case IV: Heavy usage of the Terminal with WiFi always connected <a href="#an_pm_usecase_iv" id="an_pm_usecase_iv"></a>
...
# FAQ <a href="#an_pm_faq" id="an_pm_faq"></a>

## Does Deep Sleep mode turn 3G Radio OFF? <a href="#an_pn_deepsleep_3g_off" id="an_pn_deepsleep_3g_off"></a>

No, it doesn\'t.

## Does Deep Sleep mode turn WiFi OFF? <a href="#an_pn_deepsleep_wifi_off" id="an_pn_deepsleep_wifi_off"></a>

No, it doesn\'t.

## Does Hibernate mode turn 3G Radio OFF? <a href="#an_pn_hibernate_3g_off" id="an_pn_hibernate_3g_off"></a>

Yes, it does.

## Does Hibernate mode turn WiFi off? <a href="#an_pn_hibernate_wifi_off" id="an_pn_hibernate_wifi_off"></a>

Yes, it does.

## How long does it take to resume the system from Sleep Mode? <a href="#an_pn_resume_from_sleep" id="an_pn_resume_from_sleep"></a>

It takes 1 or 2 seconds.

## How long does it take to resume the system from Hibernate? <a href="#an_pn_resume_from_hibernate" id="an_pn_resume_from_hibernate"></a>
...
## Will an application be resumed at the point it entered Hibernation? <a href="#an_pn_app_resume_from_hibernation" id="an_pn_app_resume_from_hibernation"></a>

Yes

## Is the terminal able to resume from Deep Sleep Mode automatically receiving incoming WiFi traffic? <a href="#an_pn_wifi_resume_deepsleep" id="an_pn_wifi_resume_deepsleep"></a>
...
## Can the card readers be used as wake-up sources from Standby, Sleep, etc.? <a href="#an_pn_cardreader_wakeup" id="an_pn_cardreader_wakeup"></a>
...
## Will the 24-hour auto restart happen in all power states - Sleep, Deep Sleep and Hibernate? <a href="#an_pn_24h_restart" id="an_pn_24h_restart"></a>

Yes. If no timer set for wake-up, ADK will set 24 hour reboot timer as a wake-up.

# Related Documents <a href="#an_pn_related_docs" id="an_pn_related_docs"></a>

- <a href="pg_syspm_users_guide.md">ADK-PM Programmers Guide</a>
