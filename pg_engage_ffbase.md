...
- [ADK-COM Component](pg_com_users_guide.md) for using profiles and setting up connections;
- [ADK-SYS Component](pg_sysinfo_users_guide.md) to retrieve information;
- [System mode](pg_sysmode_fltk.md) and [CCP UI](pg_ccp_users_guide.md) for access by technicians (only Engage handsets).
...
- Handset:
  ...
  - When undocked, the PM settings apply and the handset can go to the configured sleep modes. For details please see [ADK Power Management](pg_syspm_users_guide.md).
- Wake-up Scenarios:
...
All communication profiles are described in the [ADK-COM Programmer's Guide](pg_com_users_guide.md).
...
- Base Connected Status is available from the Diagnostics Application found in the Android app tray. \* The following items are viewable:
  | Field | Description |
  | -------- | -------- |
